...
export function buildSchemaOrg(content: PageContent): string {
  // Deduplicate persons by slug (team registry is already deduplicated)
  const employees = Object.entries(content.team).map(([slug, person]) => {
    // Find first proff/description for this person across services
    let jobTitle = "";
    let description = "";
    for (const service of content.resolvedServices) {
      const found = service.teammates.find((t) => t.slug === slug);
      if (found) {
  ...
  const schema = {
    ...
    sameAs: [
      "tg://resolve?domain=timamih_com",
      "https://wa.me/358453491091",
      "https://www.instagram.com/timamih_com/",
    ],
    address: {
      "@type": "PostalAddress",
